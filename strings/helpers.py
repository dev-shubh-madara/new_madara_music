HELP_1 = """<b><u>ᴀᴅᴍɪɴ ᴄᴏᴍᴍᴀɴᴅs :</u></b>

ᴊᴜsᴛ ᴀᴅᴅ <b>ᴄ</b> ᴀᴛ ᴛʜᴇ ʙᴇɢɪɴɴɪɴɢ ᴏғ ᴄᴏᴍᴍᴀɴᴅs ᴛᴏ ᴜsᴇ ᴛʜᴇᴍ ғᴏʀ ᴄʜᴀɴɴᴇʟs.

• /pause : ᴘᴀᴜsᴇs ᴛʜᴇ ᴄᴜʀʀᴇɴᴛ ᴘʟᴀʏɪɴɢ sᴛʀᴇᴀᴍ.  
• /resume : ʀᴇsᴜᴍᴇs ᴛʜᴇ ᴘᴀᴜsᴇᴅ sᴛʀᴇᴀᴍ.  
• /skip : sᴋɪᴘs ᴛʜᴇ ᴄᴜʀʀᴇɴᴛ ᴛʀᴀᴄᴋ ᴀɴᴅ sᴛᴀʀᴛs ᴛʜᴇ ɴᴇxᴛ ᴏɴᴇ.  
• /end ᴏʀ /stop : ᴄʟᴇᴀʀs ᴛʜᴇ ǫᴜᴇᴜᴇ ᴀɴᴅ sᴛᴏᴘs ᴛʜᴇ sᴛʀᴇᴀᴍ.  
• /player : ᴏᴘᴇɴs ᴀɴ ɪɴᴛᴇʀᴀᴄᴛɪᴠᴇ ᴘʟᴀʏᴇʀ ᴘᴀɴᴇʟ.  
• /queue : sʜᴏᴡs ᴛʜᴇ ǫᴜᴇᴜᴇᴅ ᴛʀᴀᴄᴋs.
"""

HELP_2 = """<b><u>ᴀᴜᴛʜ ᴜsᴇʀs :</u></b>

ᴀᴜᴛʜ ᴜsᴇʀs ᴄᴀɴ ᴜsᴇ ᴀᴅᴍɪɴ ᴄᴏᴍᴍᴀɴᴅs ᴡɪᴛʜᴏᴜᴛ ʜᴀᴠɪɴɢ ᴀᴅᴍɪɴ ʀɪɢʜᴛs ɪɴ ᴛʜᴇ ᴄʜᴀᴛ.

• /auth [ᴜsᴇʀɴᴀᴍᴇ/ɪᴅ] : ᴀᴅᴅs ᴀ ᴜsᴇʀ ᴛᴏ ᴛʜᴇ ᴀᴜᴛʜ ʟɪsᴛ.  
• /unauth [ᴜsᴇʀɴᴀᴍᴇ/ɪᴅ] : ʀᴇᴍᴏᴠᴇs ᴀ ᴜsᴇʀ ғʀᴏᴍ ᴛʜᴇ ᴀᴜᴛʜ ʟɪsᴛ.  
• /authusers : sʜᴏᴡs ᴛʜᴇ ʟɪsᴛ ᴏғ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴜsᴇʀs.
"""

HELP_3 = """<b><u>ʙʀᴏᴀᴅᴄᴀsᴛ ғᴇᴀᴛᴜʀᴇ :</u></b> [sᴜᴅᴏ ᴏɴʟʏ]

• /broadcast [ᴍᴇssᴀɢᴇ/ʀᴇᴩʟʏ] : sᴇɴᴅs ᴀ ʙʀᴏᴀᴅᴄᴀsᴛ ᴛᴏ ᴀʟʟ sᴇʀᴠᴇᴅ ᴄʜᴀᴛs.

<b>ᴍᴏᴅᴇs :</b>  
- <b>-pin</b> : ᴘɪɴs ᴛʜᴇ ᴍᴇssᴀɢᴇ.  
- <b>-pinloud</b> : ᴘɪɴs ᴀɴᴅ sᴇɴᴅs ɴᴏᴛɪғɪᴄᴀᴛɪᴏɴ.  
- <b>-user</b> : sᴇɴᴅs ᴛᴏ ᴜsᴇʀs ᴡʜᴏ sᴛᴀʀᴛᴇᴅ ᴛʜᴇ ʙᴏᴛ.  
- <b>-assistant</b> : sᴇɴᴅs ғʀᴏᴍ ᴛʜᴇ ᴀssɪsᴛᴀɴᴛ ᴀᴄᴄᴏᴜɴᴛ.  
- <b>-nobot</b> : ᴅɪsᴀʙʟᴇs ʙᴏᴛ sᴇɴᴅɪɴɢ.

<b>ᴇxᴀᴍᴩʟᴇ :</b>  
<code>/broadcast -user -assistant -pin ᴛᴇsᴛɪɴɢ ʙʀᴏᴀᴅᴄᴀsᴛ</code>
"""

HELP_4 = """<b><u>ᴄʜᴀᴛ ʙʟᴀᴄᴋʟɪsᴛ :</u></b> [sᴜᴅᴏ ᴏɴʟʏ]

ʀᴇsᴛʀɪᴄᴛ ᴀʙᴜsɪᴠᴇ ᴏʀ sᴘᴀᴍᴍʏ ᴄʜᴀᴛs ғʀᴏᴍ ᴜsɪɴɢ ᴛʜᴇ ʙᴏᴛ.

• /blacklistchat [ɪᴅ] : ʙʟᴀᴄᴋʟɪsᴛs ᴀ ᴄʜᴀᴛ.  
• /whitelistchat [ɪᴅ] : ʀᴇᴍᴏᴠᴇs ᴀ ᴄʜᴀᴛ ғʀᴏᴍ ʙʟᴀᴄᴋʟɪsᴛ.  
• /blacklistedchat : sʜᴏᴡs ᴀʟʟ ʙʟᴀᴄᴋʟɪsᴛᴇᴅ ᴄʜᴀᴛs.
"""

HELP_5 = """<b><u>ʙʟᴏᴄᴋ ᴜsᴇʀs :</u></b> [sᴜᴅᴏ ᴏɴʟʏ]

ʙʟᴏᴄᴋᴇᴅ ᴜsᴇʀs ᴄᴀɴ'ᴛ ᴜsᴇ ʙᴏᴛ ᴄᴏᴍᴍᴀɴᴅs.

• /block [ᴜsᴇʀ/ʀᴇᴩʟʏ] : ʙʟᴏᴄᴋs ᴀ ᴜsᴇʀ.  
• /unblock [ᴜsᴇʀ/ʀᴇᴩʟʏ] : ᴜɴʙʟᴏᴄᴋs ᴀ ᴜsᴇʀ.  
• /blockedusers : sʜᴏᴡs ᴀʟʟ ʙʟᴏᴄᴋᴇᴅ ᴜsᴇʀs.
"""

HELP_6 = """<b><u>ᴄʜᴀɴɴᴇʟ ᴘʟᴀʏ :</u></b>

sᴛʀᴇᴀᴍ ᴀᴜᴅɪᴏ/ᴠɪᴅᴇᴏ ɪɴ ᴀ ᴄʜᴀɴɴᴇʟ.

• /cplay : sᴛᴀʀᴛ ᴀᴜᴅɪᴏ sᴛʀᴇᴀᴍɪɴɢ.  
• /cvplay : sᴛᴀʀᴛ ᴠɪᴅᴇᴏ sᴛʀᴇᴀᴍɪɴɢ.  
• /cplayforce or /cvplayforce : ғᴏʀᴄᴇs ᴀ ɴᴇᴡ sᴛʀᴇᴀᴍ.  
• /channelplay [ᴜsᴇʀɴᴀᴍᴇ/ɪᴅ] ᴏʀ [disable] : ʟɪɴᴋs ᴀ ᴄʜᴀɴɴᴇʟ ᴡɪᴛʜ ᴀ ɢʀᴏᴜᴩ.
"""

HELP_7 = """<b><u>ɢʟᴏʙᴀʟ ʙᴀɴ :</u></b> [sᴜᴅᴏ ᴏɴʟʏ]

• /gban [ᴜsᴇʀ/ʀᴇᴩʟʏ] : ɢʟᴏʙᴀʟʟʏ ʙᴀɴs ᴀ ᴜsᴇʀ.  
• /ungban [ᴜsᴇʀ/ʀᴇᴩʟʏ] : ᴜɴʙᴀɴs ᴀ ɢʟᴏʙᴀʟʟʏ ʙᴀɴɴᴇᴅ ᴜsᴇʀ.  
• /gbannedusers : sʜᴏᴡs ᴛʜᴇ ʟɪsᴛ ᴏғ ɢʟᴏʙᴀʟʟʏ ʙᴀɴɴᴇᴅ ᴜsᴇʀs.
"""

HELP_8 = """<b><u>ʟᴏᴏᴘ sᴛʀᴇᴀᴍ :</u></b>

ʀᴇᴘᴇᴀᴛ ᴛʜᴇ ᴄᴜʀʀᴇɴᴛ sᴛʀᴇᴀᴍ.

• /loop [enable/disable] : ᴇɴᴀʙʟᴇs ᴏʀ ᴅɪsᴀʙʟᴇs ʟᴏᴏᴘ.  
• /loop [1–10] : sᴇᴛs ʟᴏᴏᴘ ᴄᴏᴜɴᴛ.
"""

HELP_9 = """<b><u>ᴍᴀɪɴᴛᴇɴᴀɴᴄᴇ :</u></b> [sᴜᴅᴏ ᴏɴʟʏ]

• /logs : ɢᴇᴛs ʙᴏᴛ ʟᴏɢs.  
• /logger [enable/disable] : ᴛᴏɢɢʟᴇs ᴀᴄᴛɪᴠɪᴛʏ ʟᴏɢɢɪɴɢ.  
• /maintenance [enable/disable] : ᴇɴᴀʙʟᴇs ᴏʀ ᴅɪsᴀʙʟᴇs ᴍᴀɪɴᴛᴇɴᴀɴᴄᴇ ᴍᴏᴅᴇ.
"""

HELP_10 = """<b><u>ᴘɪɴɢ & sᴛᴀᴛs :</u></b>

• /start : sᴛᴀʀᴛs ᴛʜᴇ ʙᴏᴛ.  
• /help : sʜᴏᴡs ᴛʜᴇ ʜᴇʟᴩ ᴍᴇɴᴜ.  
• /ping : sʜᴏᴡs ʙᴏᴛ ʟᴀᴛᴇɴᴄʏ ᴀɴᴅ sʏsᴛᴇᴍ sᴛᴀᴛs.  
• /stats : sʜᴏᴡs ᴏᴠᴇʀᴀʟʟ ʙᴏᴛ sᴛᴀᴛɪsᴛɪᴄs.
"""

HELP_11 = """<b><u>ᴩʟᴀʏ ᴄᴏᴍᴍᴀɴᴅs :</u></b>

<b>v :</b> ᴠɪᴅᴇᴏ ᴘʟᴀʏ  
<b>force :</b> ғᴏʀᴄᴇ ᴘʟᴀʏ  

• /play or /vplay : sᴛᴀʀᴛs ᴀᴜᴅɪᴏ/ᴠɪᴅᴇᴏ sᴛʀᴇᴀᴍɪɴɢ.  
• /playforce or /vplayforce : ғᴏʀᴄᴇs ᴀ ɴᴇᴡ sᴛʀᴇᴀᴍ.
"""

HELP_12 = """<b><u>sʜᴜғғʟᴇ ǫᴜᴇᴜᴇ :</u></b>

• /shuffle : sʜᴜғғʟᴇs ᴛʜᴇ ǫᴜᴇᴜᴇ.  
• /queue : sʜᴏᴡs ᴛʜᴇ sʜᴜғғʟᴇᴅ ǫᴜᴇᴜᴇ.
"""

HELP_13 = """<b><u>sᴇᴇᴋ sᴛʀᴇᴀᴍ :</u></b>

• /seek [sᴇᴄᴏɴᴅs] : ɢᴏᴇs ғᴏʀᴡᴀʀᴅ ɪɴ ᴛʜᴇ sᴛʀᴇᴀᴍ.  
• /seekback [sᴇᴄᴏɴᴅs] : sᴇᴇᴋs ʙᴀᴄᴋᴡᴀʀᴅ.
"""

HELP_14 = """<b><u>sᴏɴɢ ᴅᴏᴡɴʟᴏᴀᴅ :</u></b>

• /song [ɴᴀᴍᴇ/ʏᴛ ᴜʀʟ] : ᴅᴏᴡɴʟᴏᴀᴅs ᴀ sᴏɴɢ ɪɴ ᴍᴘ3 ᴏʀ ᴍᴘ4 ғᴏʀᴍᴀᴛ.
"""

HELP_15 = """<b><u>sᴘᴇᴇᴅ ᴄᴏᴍᴍᴀɴᴅs :</u></b> [ᴀᴅᴍɪɴ ᴏɴʟʏ]

ᴄᴏɴᴛʀᴏʟ ᴛʜᴇ ᴘʟᴀʏʙᴀᴄᴋ sᴘᴇᴇᴅ ᴏғ ᴛʜᴇ sᴛʀᴇᴀᴍ.

• /speed or /playback : ᴀᴅᴊᴜsᴛs ᴀᴜᴅɪᴏ sᴘᴇᴇᴅ ɪɴ ɢʀᴏᴜᴩs.  
• /cspeed or /cplayback : ᴀᴅᴊᴜsᴛs ᴀᴜᴅɪᴏ sᴘᴇᴇᴅ ɪɴ ᴄʜᴀɴɴᴇʟs.
"""

HELP_16 = """<b><u>ɢᴘᴛ / ᴀɪ :</u></b>

• /gpt [ǫᴜᴇsᴛɪᴏɴ] : ᴀsᴋ ᴀɪ ᴀɴʏᴛʜɪɴɢ.
• /imagine [ᴩʀᴏᴍᴩᴛ] : ɢᴇɴᴇʀᴀᴛᴇ ᴀɪ ɪᴍᴀɢᴇ.
"""

HELP_17 = """<b><u>sᴛɪᴄᴋᴇʀ ᴛᴏᴏʟs :</u></b>

• /kang [ʀᴇᴩʟʏ ᴛᴏ ᴍᴇᴅɪᴀ] : sᴛᴇᴀʟ ᴀ sᴛɪᴄᴋᴇʀ.
"""

HELP_18 = """<b><u>ᴛᴀɢ ᴀʟʟ :</u></b>

• /tagall : ᴛᴀɢs ᴀʟʟ ɢʀᴏᴜᴩ ᴍᴇᴍʙᴇʀs.
• /gmtag : ᴛᴀɢ ᴍᴇᴍʙᴇʀs ᴡɪᴛʜ ᴀ ᴍᴇssᴀɢᴇ.
"""

HELP_19 = """<b><u>ᴜsᴇʀ ɪɴғᴏ :</u></b>

• /info [ᴜsᴇʀ/ʀᴇᴩʟʏ] : ɢᴇᴛs ᴅᴇᴛᴀɪʟᴇᴅ ᴜsᴇʀ ɪɴғᴏ.
"""

HELP_20 = """<b><u>ɢʀᴏᴜᴩ ɪɴғᴏ :</u></b>

• /groupinfo : sʜᴏᴡs ɢʀᴏᴜᴩ ɪɴғᴏ ᴀɴᴅ sᴛᴀᴛs.
• /groupdata : ᴅᴏᴡɴʟᴏᴀᴅ ɢʀᴏᴜᴩ ᴅᴀᴛᴀ.
"""

HELP_21 = """<b><u>ᴇxᴛʀᴀ ᴛᴏᴏʟs :</u></b>

• /fakeinfo : ɢᴇɴᴇʀᴀᴛᴇ ғᴀᴋᴇ ᴩʀᴏғɪʟᴇ.
• /password [ʟᴇɴɢᴛʜ] : ɢᴇɴᴇʀᴀᴛᴇ sᴛʀᴏɴɢ ᴩᴀssᴡᴏʀᴅ.
• /qr [ᴛᴇxᴛ] : ɢᴇɴᴇʀᴀᴛᴇ QR ᴄᴏᴅᴇ.
• /figlet [ᴛᴇxᴛ] : ʙɪɢ ᴛᴇxᴛ ᴀsᴄɪɪ.
"""

HELP_22 = """<b><u>ɪᴍᴀɢᴇ ᴛᴏᴏʟs :</u></b>

• /img2pdf : ᴄᴏɴᴠᴇʀᴛ ɪᴍᴀɢᴇs ᴛᴏ ᴘᴅғ.
• /bgremove : ʀᴇᴍᴏᴠᴇ ʙᴀᴄᴋɢʀᴏᴜɴᴅ.
"""

HELP_23 = """<b><u>ᴀᴄᴛɪᴏɴ ᴄᴏᴍᴍᴀɴᴅs :</u></b>

• /hug, /kiss, /slap, /punch : ᴀᴄᴛɪᴏɴ ᴄᴏᴍᴍᴀɴᴅs ᴡɪᴛʜ ɢɪғs.
"""

HELP_24 = """<b><u>sᴇᴀʀᴄʜ ᴛᴏᴏʟs :</u></b>

• /ytsearch [ǫᴜᴇʀʏ] : sᴇᴀʀᴄʜ ʏᴏᴜᴛᴜʙᴇ.
• /bing [ǫᴜᴇʀʏ] : ɪᴍᴀɢᴇ sᴇᴀʀᴄʜ.
"""

HELP_25 = """<b><u>ᴀɪ / ɢᴘᴛ ᴛᴏᴏʟs :</u></b>

• /gpt [ᴩʀᴏᴍᴩᴛ] : ᴀɪ ᴄʜᴀᴛ.
• /imagine [ᴩʀᴏᴍᴩᴛ] : ᴀɪ ɪᴍᴀɢᴇ ɢᴇɴ.
"""

HELP_26 = """<b><u>ғᴜɴ ᴄᴏᴍᴍᴀɴᴅs :</u></b>

• /tictactoe : ᴩʟᴀʏ ᴛɪᴄ-ᴛᴀᴄ-ᴛᴏᴇ.
• /truthdare : ᴛʀᴜᴛʜ ᴏʀ ᴅᴀʀᴇ.
• /waifu : sᴇɴᴅ ᴀ ʀᴀɴᴅᴏᴍ ᴡᴀɪғᴜ ɪᴍᴀɢᴇ.
"""

HELP_27 = """<b><u>ᴄʀɪᴄᴋᴇᴛ ʟɪᴠᴇ :</u></b>

• /cricket : ɢᴇᴛ ʟɪᴠᴇ ᴄʀɪᴄᴋᴇᴛ sᴄᴏʀᴇs.
"""

HELP_28 = """<b><u>ᴍʏ ɪɴғᴏ :</u></b>

• /info : ɢᴇᴛ ʏᴏᴜʀ ᴏᴡɴ ᴘʀᴏғɪʟᴇ ɪɴғᴏ.
• /userinfo : ᴅᴇᴛᴀɪʟᴇᴅ ᴀᴄᴄᴏᴜɴᴛ ɪɴғᴏ.
"""

HELP_29 = """<b><u>ᴡʜᴏɪs / ᴅᴏᴍᴀɪɴ :</u></b>

• /whois [ᴅᴏᴍᴀɪɴ] : WHOIS ʟᴏᴏᴋᴜᴩ.
• /ip [ɪᴘ] : IP ɪɴғᴏ ʟᴏᴏᴋᴜᴩ.
"""

HELP_30 = """<b><u>ʀᴇᴠᴇʀsᴇ ɪᴍᴀɢᴇ :</u></b>

• /reverse [ʀᴇᴩʟʏ ᴛᴏ ɪᴍᴀɢᴇ] : ʀᴇᴠᴇʀsᴇ ɪᴍᴀɢᴇ sᴇᴀʀᴄʜ.
"""

HELP_31 = """<b><u>ɪɴsᴛᴀɢʀᴀᴍ ᴅᴏᴡɴʟᴏᴀᴅ :</u></b>

• /insta [ᴜʀʟ] : ᴅᴏᴡɴʟᴏᴀᴅ IG ᴘʜᴏᴛᴏ/ᴠɪᴅᴇᴏ.
"""

HELP_32 = """<b><u>ᴅᴏᴍᴀɪɴ ᴛᴏᴏʟs :</u></b>

• /domain [ᴅᴏᴍᴀɪɴ] : ᴄʜᴇᴄᴋ ᴅᴏᴍᴀɪɴ ɪɴғᴏ ᴀɴᴅ ᴀᴠᴀɪʟᴀʙɪʟɪᴛʏ.
"""

HELP_33 = """<b><u>ɪᴍᴀɢᴇ → ᴩᴅғ :</u></b>

• /img2pdf [ʀᴇᴩʟʏ ᴛᴏ ɪᴍᴀɢᴇs] : ᴄᴏɴᴠᴇʀᴛ ᴘʜᴏᴛᴏs ᴛᴏ PDF.
"""

HELP_34 = """<b><u>ᴩʀᴏxʏ ᴛᴏᴏʟs :</u></b>

• /proxy : ɢᴇᴛ ᴡᴏʀᴋɪɴɢ ᴩʀᴏxɪᴇs.
"""

HELP_35 = """<b><u>ᴠᴄ ᴛᴏᴏʟs :</u></b>

• /vctools : ᴍᴀɴᴀɢᴇ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ sᴇᴛᴛɪɴɢs.
• /mute / /unmute : ᴍᴜᴛᴇ/ᴜɴᴍᴜᴛᴇ ᴘᴀʀᴛɪᴄɪᴩᴀɴᴛs.
"""

HELP_36 = """<b><u>ᴀᴜᴛᴏᴩʟᴀʏ :</u></b>

• /autoplay [on/off] : ᴇɴᴀʙʟᴇ ᴀᴜᴛᴏ-ǫᴜᴇᴜᴇ ᴏɴ sᴏɴɢ ᴇɴᴅ.
"""

HELP_37 = """<b><u>⚔️ ᴄʜᴀᴛ ғɪɢʜᴛ — ᴡᴏʀᴅ ɢᴀᴍᴇs :</u></b>

• /chatfight or /cf : sᴛᴀʀᴛ ᴀ ᴡᴏʀᴅ/ᴇᴍᴏᴊɪ/ғʟᴀɢ ɢᴀᴍᴇ.
• /gametop : sʜᴏᴡ ɢʟᴏʙᴀʟ ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ.

ᴛʜʀᴇᴇ ɢᴀᴍᴇ ᴛʏᴘᴇs :
  🔤 ᴡᴏʀᴅ — ᴜɴsᴄʀᴀᴍʙʟᴇ ᴛʜᴇ ᴊᴜᴍʙʟᴇᴅ ᴡᴏʀᴅ.
  😀 ᴇᴍᴏᴊɪ — ɢᴜᴇss ᴡʜᴀᴛ ᴛʜᴇ ᴇᴍᴏᴊɪ ʀᴇᴩʀᴇsᴇɴᴛs.
  🌍 ғʟᴀɢ — ɢᴜᴇss ᴛʜᴇ ᴄᴏᴜɴᴛʀʏ ғʀᴏᴍ ɪᴛs ғʟᴀɢ.

ᴄᴏʀʀᴇᴄᴛ ᴀɴsᴡᴇʀ = +10 ᴩᴏɪɴᴛs ɪɴ ɢʟᴏʙᴀʟ ʀᴀɴᴋɪɴɢ!
"""

HELP_38 = """<b><u>🔑 sᴛʀɪɴɢ sᴇssɪᴏɴ ɢᴇɴᴇʀᴀᴛᴏʀ :</u></b>

• /genstring : ɢᴇɴᴇʀᴀᴛᴇ sᴛʀɪɴɢ sᴇssɪᴏɴ ɪɴ ᴘʀɪᴠᴀᴛᴇ.

sᴜᴘᴩᴏʀᴛᴇᴅ ᴛʏᴩᴇs :
  🔥 <b>Pyrogram V2</b> — ᴩʏʀᴏғᴏʀᴋ / ᴍᴏᴅᴇʀɴ ʙᴏᴛs
  ⭐ <b>Pyrogram V1</b> — ʟᴇɢᴀᴄʏ ʙᴏᴛs
  ⚡ <b>Telethon</b> — ᴛᴇʟᴇᴛʜᴏɴ ʙᴀsᴇᴅ ʙᴏᴛs

100% sᴇᴄᴜʀᴇ — sᴇssɪᴏɴs ᴀʀᴇ ɴᴇᴠᴇʀ sᴛᴏʀᴇᴅ!
"""

HELP_39 = """<b><u>🏆 ɢᴀᴍᴇ ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ :</u></b>

• /gametop : sʜᴏᴡ ᴛᴏᴩ 10 ᴩʟᴀʏᴇʀs ɢʟᴏʙᴀʟʟʏ.
• /wordleaderboard : ᴀʟɪᴀs ғᴏʀ /gametop.

ᴡɪɴ ᴄʜᴀᴛ ғɪɢʜᴛ ɢᴀᴍᴇs ᴛᴏ ᴄʟɪᴍʙ ᴛʜᴇ ʀᴀɴᴋs!
"""

HELP_40 = """<b><u>💻 ɢɪᴛʜᴜʙ ᴍᴀɴᴀɢᴇʀ :</u></b>

• /github — sʜᴏᴡ ᴍᴀɪɴ ᴍᴇɴᴜ
• /gitinfo <code>username</code> — ᴜsᴇʀ ᴩʀᴏғɪʟᴇ
• /gitrepo <code>owner/repo</code> — ʀᴇᴩᴏ ɪɴғᴏ
• /zrepo <code>owner/repo</code> [token] — ᴅᴏᴡɴʟᴏᴀᴅ ᴢɪᴩ
• /zpush <code>owner/repo</code> <code>token</code> — ᴜᴩʟᴏᴀᴅ ᴢɪᴩ ᴛᴏ ʀᴇᴩᴏ
• /gitissues <code>owner/repo</code> — ᴏᴩᴇɴ ɪssᴜᴇs
• /gitcommits <code>owner/repo</code> — ʀᴇᴄᴇɴᴛ ᴄᴏᴍᴍɪᴛs
• /gitstar <code>owner/repo</code> — sᴛᴀʀ ᴀ ʀᴇᴩᴏ
• /gitfork <code>owner/repo</code> — ғᴏʀᴋ ᴀ ʀᴇᴩᴏ
• /gitbranch <code>owner/repo</code> — ʟɪsᴛ ʙʀᴀɴᴄʜᴇs
• /gittoken <code>token</code> — sᴀᴠᴇ ʏᴏᴜʀ ᴛᴏᴋᴇɴ (ᴩᴍ ᴏɴʟʏ)
"""
